...
lixingr 包的主函数是 `lxr_query()`，你可以在 Console 中输入 `?lxr_query` 后回车，以查看函数文档。
...
为避免复述接口端点参数衍生的工作量（其实是偷懒），需要你在[理杏仁接口文档](https://www.lixinger.com/open/api/)查看对应端点的参数说明，并为 `lxr_query()` 提供参数。参数使用驼峰命名法，或是下划线命名法，这取决于你的喜好，如 `lxr_query()` 文档所说两者皆可。

`lxr_query()` 会帮你检查输入参数是否合法、必要参数是否缺失。在理杏仁接口文档标识为数组的参数，你只需要以字符向量的形式输入即可。函数会自动帮你转换。

比如，若以理杏仁[大陆上市公司基础信息接口端点文档](https://www.lixinger.com/open/api/doc?api-key=cn/company/profile)中的请求方式同时查询多家大陆上市公司基础信息，你需要这样写：
