...
密码为 ECS 的登录密码，如提示密码错误，到 ECS 密码即可。

登录成功，你应看到如下界面：

![](image/jihuo.png){fig-align="center"}

如果上述 URL 不能访问到如上页面，还可尝试 `https://ECS 公网 IP:8850/`。

点击“启动 Tableau Server 试用版”，之后应有如下界面：

![](image/zhuce.png){fig-align="center"}

按实际情况填写后，点击“注册”，之后应有如下界面：
...
点击“继续”，应当跳转到新页面设置 Tableau Server 管理员账户，但笔者这里跳转失败。显示如下提示：

![](image/anzhuangwwc.png){fig-align="center"}

不用慌，我们只需要远程连接 ECS，在命令行下用命令创建初始管理员用户即可：

```sh
tabcmd initialuser --server http://localhost --username 'tableau-admin'
```

运行该命令之后，命令行将提示输入管理密码。设置好密码后。

## 登录 Tableau Server

使用 `http://ECS 公网 IP` 或 `https://ECS 公网 IP`,应能访问到如下界面：

![](image/guanliyuanlogin.png)

使用刚才设置的管理员用户名及密码，登录成功之后应有如下界面：
